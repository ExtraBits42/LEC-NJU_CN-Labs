...
def test_myswitch_to():
    ...
    s.expect(
        ...
        ("eth1 received an Ethernet frame with a destination address which the switch have learned"
    ...
    s.expect(
        PacketInputTimeoutEvent(11.0),
        ("waiting fo 11s")
    )
    
    # test case 5：the specific item expired!
    s.expect(
        PacketInputEvent('eth1', pkt1to0, display=Ethernet),
        ("eth1 received an Ethernet frame with a destination address which the switch have learned"
         "should forward out specific port eth0")
    ...
    s.expect(
        PacketOutputEvent("eth0", pkt1to0, "eth2", pkt1to0, display=Ethernet),
        ("The Ethernet frame should forward out port eth0")
    )
    return s


scenario = test_myswitch_to()